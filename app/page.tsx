...
export default async function Home() {
  ...
  return (
    <main style={{ background: 'var(--bg-primary)', minHeight: '100vh', fontFamily: "'Nunito Sans', sans-serif" }}>
      ...
      <section className="px-5 sm:px-14 pt-16 sm:pt-24 pb-14 sm:pb-20 max-w-[1200px] mx-auto">
        ...
        <h1 className="font-bold mb-6 sm:mb-7"
          style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(40px, 8vw, 72px)', lineHeight: 1.05, letterSpacing: '-2px', maxWidth: '800px', color: 'var(--text-primary)' }}>
          Senior AECO expertise,{' '}
          <span style={{ color: '#F6981F', fontStyle: 'italic' }}>fractional</span>{' '}by design.
        </h1>
        <p className="mb-10 sm:mb-12 max-w-lg" style={{ fontSize: 'clamp(16px, 2.5vw, 20px)', color: 'var(--text-muted)', lineHeight: 1.7, fontWeight: 400 }}>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
          ...
          <a href="/talent" className="font-semibold text-base sm:text-lg px-6 py-4 rounded-xl w-full sm:w-auto text-center hover:opacity-80 transition-opacity"
            style={{ background: 'transparent', color: 'var(--text-muted)', textDecoration: 'none', border: '1.5px solid var(--border-color)' }}>
            Browse talent →
          </a>
        </div>

        {/* Live stats — 3 across */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 sm:gap-0 mt-16 sm:mt-20 pt-10 sm:pt-12 border-t" style={{ borderColor: 'var(--border-color)' }}>
          {stats.map((s, i) => (
            <div key={i} className="sm:pr-14 sm:mr-14 sm:border-r last:sm:border-r-0 last:sm:mr-0 last:sm:pr-0"
              style={{ borderColor: 'var(--border-color)' }}>
              <div className="font-bold" style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(28px, 5vw, 40px)', fontWeight: 800, color: 'var(--text-primary)' }}>{s.value}</div>
              <div className="text-xs sm:text-sm font-semibold mt-1" style={{ color: 'var(--text-secondary)' }}>{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* How it works */}
      <section id="how" className="px-5 sm:px-14 py-16 sm:py-24 border-t" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        <div className="max-w-[1200px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-20 items-start mb-12 sm:mb-16">
            <div>
              ...
              <h2 className="font-bold" style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(28px, 5vw, 48px)', lineHeight: 1.1, letterSpacing: '-1px', margin: 0, color: 'var(--text-primary)' }}>
                From profile to project, in three moves.
              </h2>
            </div>
            <div className="md:pt-10">
              <p style={{ color: 'var(--text-muted)', fontSize: 'clamp(15px, 2vw, 18px)', lineHeight: 1.7 }}>
                Built for senior practitioners who&apos;d rather execute than chase work, and for teams that need expertise on demand.
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 border-t" style={{ borderColor: 'var(--border-color)' }}>
            {[
              ...
              <div key={i} className="py-10 sm:py-12 md:border-r last:md:border-r-0"
                style={{ paddingLeft: i > 0 ? undefined : '0', paddingRight: '40px', borderColor: 'var(--border-color)' }}>
                <div className="flex items-center justify-between mb-6">
                  <span className="text-2xl sm:text-3xl">{s.icon}</span>
                  <span className="text-xs sm:text-sm font-bold" style={{ color: 'var(--border-color)' }}>{s.num}</span>
                </div>
                <h3 className="font-bold mb-3 text-lg sm:text-xl" style={{ fontFamily: "'Nunito', sans-serif", color: 'var(--text-primary)' }}>{s.title}</h3>
                <p className="text-sm sm:text-base" style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Featured talent */}
      <section className="px-5 sm:px-14 py-16 sm:py-24 border-t" style={{ background: 'var(--bg-primary)', borderColor: 'var(--border-color)' }}>
        <div className="max-w-[1200px] mx-auto">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-10 sm:mb-12">
            <div>
              ...
              <h2 className="font-bold" style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(28px, 5vw, 48px)', letterSpacing: '-1px', margin: 0, color: 'var(--text-primary)' }}>
              ...
              <a key={i} href="/talent" className="block p-5 sm:p-7 rounded-2xl hover:opacity-90 transition-opacity"
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold text-sm sm:text-base" style={{ color: 'var(--text-primary)' }}>{p.name}</div>
                      <div className="text-xs sm:text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>{p.role}</div>
                    </div>
                  </div>
                ...
                <div className="flex gap-2 mb-4 flex-wrap">
                  {p.tags.map((tag, j) => (
                    <span key={j} className="text-xs px-3 py-1 rounded-md font-semibold"
                      style={{ border: '1px solid var(--border-color)', color: 'var(--text-muted)' }}>{tag}</span>
                  ))}
                </div>
                <div className="flex justify-between items-center pt-4 border-t" style={{ borderColor: 'var(--border-color)' }}>
                  <span className="text-xs sm:text-sm font-semibold" style={{ color: 'var(--text-secondary)' }}>{p.exp} experience</span>
      ...
      {/* Integrations */}
      <section id="integrations" className="px-5 sm:px-14 py-16 sm:py-24 border-t" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
        <div className="max-w-[1200px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center">
          <div>
            ...
            <h2 className="font-bold mb-5" style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(26px, 4.5vw, 44px)', lineHeight: 1.1, letterSpacing: '-1px', color: 'var(--text-primary)' }}>
              Your career, already written.{' '}
              <span style={{ color: '#F6981F', fontStyle: 'italic' }}>Just import it.</span>
            </h2>
            <p className="mb-8 text-sm sm:text-lg" style={{ color: 'var(--text-muted)', lineHeight: 1.7 }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {['LinkedIn', 'Indeed', 'Glassdoor', 'AngelList', 'CV upload'].map((p, i) => (
                <span key={i} className="px-4 py-2 rounded-lg text-xs sm:text-sm font-bold"
                  style={{ background: i === 0 ? '#F6981F' : 'var(--bg-primary)', color: i === 0 ? 'white' : 'var(--text-secondary)', border: `1px solid ${i === 0 ? '#F6981F' : 'var(--border-color)'}` }}>
                  {p}
                </span>
              ))}
            </div>
          </div>
          <div className="rounded-2xl p-6 sm:p-8" style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-color)' }}>
            <div className="flex items-center gap-2 mb-6">
              <div className="w-2 h-2 rounded-full" style={{ background: '#05809B' }}></div>
              <span className="text-xs sm:text-sm font-semibold" style={{ color: 'var(--text-secondary)', fontFamily: 'monospace' }}>Importing from LinkedIn…</span>
            </div>
            {[
              ...
              <div key={i} className="flex items-center gap-3 sm:gap-4 py-3 sm:py-4"
                style={{ borderBottom: i < 2 ? `1px solid var(--bg-card)` : 'none' }}>
                <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl flex-shrink-0" style={{ background: r.color, opacity: 0.8 }}></div>
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm sm:text-base truncate" style={{ color: 'var(--text-primary)' }}>{r.title}</div>
                  <div className="text-xs sm:text-sm font-semibold mt-0.5 truncate" style={{ color: 'var(--text-secondary)' }}>{r.company}</div>
                </div>
                <span className="text-xs font-semibold flex-shrink-0" style={{ color: 'var(--border-color)' }}>{r.years}</span>
              </div>
            ))}
      ...
      <section className="px-5 sm:px-14 py-16 sm:py-24 border-t" style={{ background: 'var(--bg-primary)', borderColor: 'var(--border-color)' }}>
        <div className="max-w-[1200px] mx-auto">
          ...
          <h2 className="font-bold text-center mb-12 sm:mb-14" style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(28px, 5vw, 48px)', letterSpacing: '-1px', color: 'var(--text-primary)' }}>Our Services</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="rounded-2xl p-7 sm:p-9" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
      ...
      {/* Footer */}
      <footer className="px-5 sm:px-14 pt-12 sm:pt-14 pb-8 sm:pb-10 border-t" style={{ background: 'var(--bg-card)', borderColor: 'var(--border-color)', fontFamily: "'Nunito Sans', sans-serif" }}>
        <div className="max-w-[1200px] mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-start gap-10 mb-10 sm:mb-12">
            <div>
              <div className="font-bold text-xl sm:text-2xl" style={{ fontFamily: "'Nunito', sans-serif", color: 'var(--text-primary)' }}>Fractus</div>
              <div className="text-xs sm:text-sm font-semibold mt-1" style={{ color: 'var(--text-secondary)' }}>A Fractional AECO product</div>
            ...
            <div className="flex gap-10 sm:gap-14 flex-wrap">
              <div>
                <div className="text-xs font-bold tracking-widest mb-4" style={{ color: 'var(--border-color)' }}>PLATFORM</div>
                {[['Browse Talent', '/talent'], ['Listings', '/listings'], ['Create Profile', '/signup'], ['Sign In', '/login']].map(([label, href]) => (
                  <a key={label as string} href={href as string}
                    className="block text-sm font-semibold mb-2 hover:opacity-80 transition-opacity"
                    style={{ color: 'var(--text-muted)', textDecoration: 'none' }}>{label}</a>
                ))}
              </div>
              <div>
                <div className="text-xs font-bold tracking-widest mb-4" style={{ color: 'var(--border-color)' }}>COMPANY</div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 pt-6 border-t" style={{ borderColor: 'var(--border-color)' }}>
            <div className="text-xs sm:text-sm font-semibold" style={{ color: 'var(--text-secondary)' }}>© 2026 Fractional-AECO LLC. All rights reserved.</div>
            <a href="tel:+19804940263" className="text-xs sm:text-sm font-semibold hover:opacity-80 transition-opacity"
              style={{ color: 'var(--text-secondary)', textDecoration: 'none' }}>+1 980 494 0263</a>
          </div>
        </div>
      </footer>
