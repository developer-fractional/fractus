  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', fontFamily: "'Nunito Sans', sans-serif" }}>
      ...
      <div style={{ borderBottom: '1px solid var(--border-color)', background: 'var(--bg-card)', padding: 'clamp(32px,5vw,52px) 20px' }}>
        <div style={{ maxWidth: '960px', margin: '0 auto' }}>
          ...
          <div>
            ...
            <h1 style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(26px,5vw,38px)', fontWeight: 800, color: 'var(--text-primary)' }}>Saved Listings</h1>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: 'clamp(32px,4vw,48px) 20px' }}>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '80px', color: 'var(--text-secondary)', fontSize: '18px' }}>Loading...</div>

        ) : rows.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>🔖</div>
            <h3 style={{ fontFamily: "'Nunito', sans-serif", fontSize: '22px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '10px' }}>No saved listings yet</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '15px', marginBottom: '28px' }}>Browse open roles and tap 🔖 to save ones you want to revisit.</p>
            <Link href="/listings"
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {rows.map(row => {
                ...
                if (!l) return null
                return (
                  <div key={row.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: 'clamp(20px,3vw,28px)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
                      ...
                      <span style={{
                        ...
                        color:      l.status === 'active' ? '#F6981F' : 'var(--text-secondary)',
                        border:     l.status === 'active' ? '1px solid rgba(246,152,32,0.3)' : '1px solid var(--border-color)',
                      }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginBottom: '16px' }}>
                      ...
                      {l.created_at && <span style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>🗓 {formatDate(l.created_at)}</span>}
                    </div>
