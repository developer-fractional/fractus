...
export default function TalentPage() {
  ...
  const filterBar: React.CSSProperties = { borderBottom: '1px solid var(--border-color)', padding: '14px 0' }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', fontFamily: "'Nunito Sans', sans-serif" }}>
      ...
      <div style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border-color)', padding: 'clamp(40px,6vw,64px) 20px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <h1 style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(32px,6vw,48px)', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '12px' }}>Browse AECO Talent</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '18px', marginBottom: '28px' }}>Senior practitioners available fractionally. Vetted by FractionalAECO.</p>
      ...
      {/* Filter bar: Availability + Sort */}
      <div style={filterBar}>
        <div style={{ ...rowWrap, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <span style={{ color: 'var(--text-secondary)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', alignSelf: 'center', marginRight: '4px', textTransform: 'uppercase' }}>Availability</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ color: 'var(--text-secondary)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Sort</span>
      ...
      <div style={{ ...filterBar, display: 'flex', alignItems: 'center' }}>
        <div style={{ ...rowWrap, display: 'flex', alignItems: 'center', gap: '10px', width: '100%' }}>
          <span style={{ color: 'var(--text-secondary)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Location</span>
          <input type="text" placeholder="Filter by location..." value={filterLocation} onChange={e => setFilterLocation(e.target.value)}
            style={{ flex: 1, maxWidth: '320px', background: 'var(--input-bg)', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '8px 14px', color: 'var(--text-primary)', fontSize: '13px', outline: 'none' }} />
          {activeFilterCount > 0 && (
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: 'clamp(32px,4vw,48px) 20px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '80px', color: 'var(--text-secondary)', fontSize: '18px' }}>Loading talent...</div>
        ) : showEmptyState ? (
          <div style={{ textAlign: 'center', padding: '80px 20px' }}>
            <div style={{ fontSize: '48px', marginBottom: '16px' }}>🔍</div>
            <h3 style={{ fontFamily: "'Nunito', sans-serif", fontSize: '22px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '10px' }}>No professionals found</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '15px', marginBottom: '28px' }}>No professionals match your current filters. Try adjusting your search.</p>
                ...
                const cardContent = (
                  <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                      <h3 style={{ fontFamily: "'Nunito', sans-serif", fontSize: '17px', fontWeight: 800, color: 'var(--text-primary)' }}>{p.name}</h3>
                      {p.is_verified && (
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '14px' }}>{p.role}</p>

                    {p.skills && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
                        {p.skills.split(',').slice(0, 2).map((s: string, j: number) => (
                          <span key={j} style={{ fontSize: '12px', padding: '4px 10px', borderRadius: '100px', background: 'var(--bg-primary)', color: '#05809B', border: '1px solid var(--border-color)' }}>{s.trim()}</span>
                        ))}
                      </div>
                    )}
                    ...
                    {(p.certifications?.length ?? 0) > 0 && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '10px' }}>
                        {(p.certifications ?? []).slice(0, 3).map((c: string, j: number) => (
                          <span key={j} style={{ fontSize: '12px', padding: '4px 10px', borderRadius: '100px', background: 'var(--bg-primary)', color: 'var(--text-muted)', border: '1px solid var(--border-color)' }}>{c}</span>
                        ))}
                      </div>
                    )}

                    {p.location && (
                      <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginBottom: '14px' }}>📍 {p.location}</p>
                    )}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '14px', borderTop: '1px solid var(--border-color)' }}>
                      <span style={{ color: 'var(--text-secondary)', fontSize: '13px' }}>{p.years_experience} yrs exp</span>
      ...
      {/* CTA */}
      <div style={{ textAlign: 'center', padding: 'clamp(48px,6vw,64px) 20px', borderTop: '1px solid var(--border-color)' }}>
        <h3 style={{ fontFamily: "'Nunito', sans-serif", fontSize: 'clamp(22px,4vw,32px)', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '12px' }}>Need senior AECO firepower?</h3>
        <p style={{ color: 'var(--text-muted)', fontSize: '17px', marginBottom: '32px' }}>Tell us what you need. We&apos;ll match you with vetted senior practitioners.</p>
        <div style={{ display: 'flex', gap: '14px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link href="/signup"
            style={{ border: '1.5px solid var(--border-color)', color: 'var(--text-muted)', textDecoration: 'none', borderRadius: '100px', padding: '16px 32px', fontSize: '16px', fontWeight: 600 }}>
            Join as talent
          </Link>
        </div>
      </div>
